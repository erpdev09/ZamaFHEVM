...
async function registerRoutesInternal(app: Express, skipFileServing: boolean = false): Promise<void> {
  ...
  app.get("/api/metrics/:originId", async (req, res) => {
    try {
      ...
      // Time series (simplified for MVP)
      const timeSeries = [
        { date: "2025-01-18", visitors: Math.floor(visitors * 0.7), pageviews: Math.floor(pageviews * 0.7) },
        { date: "2025-01-19", visitors: Math.floor(visitors * 0.8), pageviews: Math.floor(pageviews * 0.8) },
        { date: "2025-01-20", visitors: Math.floor(visitors * 0.85), pageviews: Math.floor(pageviews * 0.85) },
        { date: "2025-01-21", visitors: Math.floor(visitors * 0.95), pageviews: Math.floor(pageviews * 0.95) },
        { date: "2025-01-22", visitors, pageviews },
      ];
